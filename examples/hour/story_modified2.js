...
// Modified file using the method of implementing body parts as attributes of a Body object that belongs to each Actor.
...
thing.jBody = actor.josephine.getBody(); // josephine's body. some method that gets the Body object that belongs to the Actor, which contains the body parties
thing.lBody = actor.louise.getBody(); // louise's body. same as above, stored as a Thing in the story (I feel uneasy with this naming but perhaps there's a workaround with the naming)
...
thing.pulse = new Thing(thing.lBody.arms);
...
ev.weep = new Event(actor.louise, null, temporal.into, thing.jBody.arms);
...
ev.openArms = new Event(actor.louise, thing.lBody.arms);
